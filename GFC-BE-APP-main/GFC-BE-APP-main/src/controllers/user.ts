...
export const getUserByIdController = (req: Request, res: Response) => {
  Users.find({ username: req.params.id })
    ...
    .catch((error: any) => {
      ...
      res.status(500).send({ message: REQUEST_FAILURE_MESSAGES.ERROR_IN_FETCHING_USER_DATA });
    });
}
  ...
  try {
    ...
    Users.find({ email: req.body.email }).then((response: any) => {
      if (response.length > 0) {
        return res.status(403).send(REQUEST_FAILURE_MESSAGES.USER_ALREADY_EXISTS);
      }
...
export const signInUserController = (req: Request, res: Response) => {
  ...
  Users.find({ email: email }).then((user: any) => {
    if (user.length == 0) {
      ...
      return res.status(403).send({ message: REQUEST_FAILURE_MESSAGES.USER_DATA_NOT_FOUND });
    }
    ...
    bcrypt.compare(password, user[0].password)
      .then((isMatched: boolean) => {
        if (!isMatched) {
          return res.status(402).send({ message: REQUEST_FAILURE_MESSAGES.PASSWORD_INCORRECT });
        }
        ...
        logger.info(REQUEST_SUCCESS_MESSAGE.USER_LOGGEDIN_SUCCESSFULLY, { email: userEmail, username });
        res.status(200).json({
          message: LOGGED_IN,
          token: token,
          userId: _id.toString(),
