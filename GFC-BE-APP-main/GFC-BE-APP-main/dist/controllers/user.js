...
const getUserByIdController = (req, res) => {
    Users.find({ username: req.params.id })
        ...
        res.status(500).send({ message: "Error in fetching the user data," /* REQUEST_FAILURE_MESSAGES.ERROR_IN_FETCHING_USER_DATA */ });
    });
};
    ...
    try {
        ...
        Users.find({ email: req.body.email }).then((response) => {
            if (response.length > 0) {
                return res.status(403).send("User already exists, try new email address" /* REQUEST_FAILURE_MESSAGES.USER_ALREADY_EXISTS */);
            }
            bcrypt.hash(user.password, 12).then((hashedPassword) => {
                ...
                user.save().then((response) => {
                    ...
                    pino_1.logger.info("User logged in successfully" /* REQUEST_SUCCESS_MESSAGE.USER_LOGGEDIN_SUCCESSFULLY */, { email: response.email, username: response.username });
...
const signInUserController = (req, res) => {
    ...
    Users.find({ email: email }).then((user) => {
        if (user.length == 0) {
            ...
            return res.status(403).send({ message: "User data not found..!" /* REQUEST_FAILURE_MESSAGES.USER_DATA_NOT_FOUND */ });
        }
        const { email: userEmail, username, _id } = user[0];
        bcrypt.compare(password, user[0].password)
            .then((isMatched) => {
            if (!isMatched) {
                return res.status(402).send({ message: "Password is incorrect..!" /* REQUEST_FAILURE_MESSAGES.PASSWORD_INCORRECT */ });
            }
            const token = jwt.sign({
                email: userEmail,
                username,
            ...
            pino_1.logger.info("User logged in successfully" /* REQUEST_SUCCESS_MESSAGE.USER_LOGGEDIN_SUCCESSFULLY */, { email: userEmail, username });
            res.status(200).json({
                message: LOGGED_IN,
                token: token,
                userId: _id.toString(),
